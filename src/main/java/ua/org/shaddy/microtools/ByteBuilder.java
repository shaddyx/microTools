...
public class ByteBuilder {
	...
	// all append() methods return this so we can chain calls together, of
	// course :)
	...
	public ByteBuilder append(Object value) {
		try {
			// attempts to call a toByteStructure() method on the object, if it
			// has it.
		...
		// if the above did not work, instead append the result of the
		// toString() method.
			...
			for (int i = 0; i < length; i++) {
				...
				if (preserveNegative && i == 0) {
					if (value < 0) { // original value is negative
						b[i] |= 0x80; // 1000 0000 - or'ed into the value to
										// make it negative
					} else { // original value is positive
						b[i] &= 0x7F; // 0111 0000 - and'ed into the value to
